...
program main
    use avldef
    type(sucursal) :: s1, s2, s3, s4, s5, s6, s7, s8, s9, s10
    type(avl) :: avl1
    class(node), pointer :: res, res2
    integer :: unit
        character(len=100) :: filename
    filename = 'output.dot'
        open(unit, file=filename, status='replace')
    s1 = sucursal(5, 'Sucursal 1', 'Direccion 1', 'Telefono 1', 'Email 1')
    s2 = sucursal(10, 'Sucursal 2', 'Direccion 2', 'Telefono 2', 'Email 2')
    s3 = sucursal(15, 'Sucursal 3', 'Direccion 3', 'Telefono 3', 'Email 3')
    s4 = sucursal(20, 'Sucursal 4', 'Direccion 4', 'Telefono 4', 'Email 4')
    s5 = sucursal(25, 'Sucursal 5', 'Direccion 5', 'Telefono 5', 'Email 5')
    s6 = sucursal(30, 'Sucursal 6', 'Direccion 6', 'Telefono 6', 'Email 6')
    s7 = sucursal(35, 'Sucursal 7', 'Direccion 7', 'Telefono 7', 'Email 7')

    print *, 'Adding elements'
    call avl1%add(s1)
    call avl1%add(s2)
    call avl1%add(s3)
    call avl1%preorder(avl1%root)
    call avl1%add(s4)
    call avl1%add(s5)
    call avl1%add(s6)
    print *, 'Elements added 7'
    call avl1%add(s7)
    ...
    print *, 'Searching for sucursal 5'
    
    res => avl1%searchSucursalID(5)
    if (associated(res))     then
        print *, 'Sucursal found:', res%value%nombre
        !cambiarle nombre
        res%value%nombre = 'Sucursal 5 Modificada'
    else
        print *, 'Sucursal not found'
    end if

    res2 => avl1%searchSucursalID(5)
    if (associated(res2)) then
        print *, 'Sucursal found:', res2%value%nombre
    else
        print *, 'Sucursal not found'
    end if
