module module_routes
    ...
    subroutine graphic(this)
        ...
        ! windows
        ! call system("start images\route.png")
        !linux
...
module module_jsonReaderGrafos
    use json_module
    use TableHash
    ...
    end type jsonReaderGrafos

    contains

    subroutine readJsonGrafos(this, file_route,graph_read,tree,avl1) 
        class(jsonReaderGrafos), intent(inout) :: this
        type(graph), intent(inout) :: graph_read
        type(merkle), intent(inout) :: tree
        ...
        logical :: found    
        character(len=100) :: nombreSucursalInicial, nombreSucursalFinal
        ...
        do i = 1, size
            ...
            print *, "S1: ", node_o, " S2: ", node_d, " Distancia: ", distance, " Impresoras: ", printers
            call graph_read%insert_data(node_o, node_d, distance)
            nombreSucursalInicial =  avl1%getNameSucursalId(node_o)
            nombreSucursalFinal =  avl1%getNameSucursalId(node_d)
            print *, "Nombre Sucursal Inicial: ", trim(adjustl(nombreSucursalInicial)), &
            " Nombre Sucursal Final: ", trim(adjustl(nombreSucursalFinal))
            call tree%add_data(node_o, trim(adjustl(nombreSucursalInicial)), node_d, trim(adjustl(nombreSucursalFinal)), 0)

        end do

        call json%destroy()        
    end subroutine readJsonGrafos
